...
- Etapa atual: pipeline robusto com MLP em PyTorch, automação, rastreabilidade em MLflow e refatoração estrutural da base de código
...
O código do projeto iniciou uma refatoração estrutural para uma arquitetura modular em `src/churn_prediction`.

Escopo desta etapa:

- criação da base de pacotes do projeto;
- separação inicial entre configuração, núcleo compartilhado, dados, features, modelagem, neural, pipelines e orquestração;
- preparação da base para pipeline reproduzível, testes, API e observabilidade.

Impacto esperado nesta etapa:

- nenhum impacto intencional nas métricas do modelo;
- foco em organização, rastreabilidade, reprodutibilidade e manutenção do código.
...
- A refatoração para módulos ainda está em andamento; parte da lógica permanece em notebooks nesta etapa
...
1. Migrar progressivamente a lógica dos notebooks para módulos reutilizáveis.
2. Implementar pipeline reproduzível em `src/churn_prediction/pipelines/`.
3. Adicionar testes automatizados e validação de schema.
4. Criar camada de inferência via FastAPI.
5. Avaliar fairness do MLP por subgrupos sensíveis.
6. Definir política de monitoramento de performance e fairness em produção.